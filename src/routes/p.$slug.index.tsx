...
export const Route = createFileRoute("/p/$slug/")({
  head: (ctx: { params: { slug: string }; loaderData?: unknown }) => {
    ...
    const url = `https://www.frontiers3d.com/p/${ctx.params.slug}`;
    const image = b?.hero_bg_url || b?.logo_url || "https://www.frontiers3d.com/og-3d-presentation-studio.png";
...
function PortalPage() {
  ...
  // (shared with /p/$slug/builder).
  useBrandedFavicon(branding?.favicon_url, branding?.logo_url);
  ...
  const heroLines = (() => {
    ...
    // Prefer the proper hero_lines JSONB column; fall back to
    // calling_card_headline for records saved under the earlier workaround.
    const raw = b.hero_lines ?? b.calling_card_headline;
    let arr: unknown[] | null = null;
    if (Array.isArray(raw)) {
      arr = raw;
    } else if (typeof raw === "string" && raw.startsWith("[")) {
      try { arr = JSON.parse(raw); } catch { /* invalid JSON */ }
    }
