...
export const STARTER_TEMPLATES: StarterTemplate[] = [
  ...
  },
  {
    ...
    description:
      "Comprehensive event venue template covering capacity by setup style, access & operations, accessibility, wedding/party specifics, AV & tech, on-site dining, and atmosphere amenities. Built for venue fact sheets, planner inquiries, and tour decks.",
    icon: "Building2",
    ...
    schema: {
      ...
      properties: {
        // Capacity & Space
        max_total_capacity: { type: "number", description: "Maximum total guest capacity across all configurations" },
        banquet_capacity: { type: "number", description: "Capacity for sit-down banquet (round tables) seating" },
        cocktail_capacity: { type: "number", description: "Capacity for standing / cocktail-reception style" },
        theater_capacity: { type: "number", description: "Capacity for theater-style seating (rows of chairs)" },
        classroom_capacity: { type: "number", description: "Capacity for classroom-style seating (tables and chairs)" },
        total_sqft: { type: "number", description: "Total event space square footage" },
        ceiling_height_ft: { type: "number", description: "Main room ceiling height in feet" },
        breakout_rooms_count: { type: "number", description: "Number of breakout rooms or private areas" },
        // Location & Access
        ...
        onsite_parking_spaces: { type: "number", description: "Number of on-site parking spaces" },
        valet_parking_available: { type: "boolean", description: "True if valet parking is offered" },
        transit_distance_min: { type: "number", description: "Walking minutes to nearest public transit station" },
        loading_dock_access: { type: "string", description: "Loading dock and vendor equipment entry details" },
        // Operations & Rules
        loadin_loadout_windows: { type: "string", description: "Allowed load-in and load-out time windows" },
        event_duration_limit: { type: "string", description: "Event duration limits (hourly cap or per-event)" },
        noise_curfew: { type: "string", description: "Noise ordinances and curfew times" },
        exclusive_catering_required: { type: "boolean", description: "True if venue chef / in-house catering is mandatory" },
        outside_vendor_policy: { type: "string", description: "Policy on outside vendors (florist, DJ, planner)" },
        liquor_license_status: { type: "string", description: "Liquor license status held by the venue" },
        bar_rules: { type: "string", description: "Bar service rules and restrictions" },
        host_insurance_required: { type: "boolean", description: "True if hosts must provide event insurance" },
        booking_deposit_terms: { type: "string", description: "Booking deposit amount and schedule" },
        cancellation_policy: { type: "string", description: "Cancellation and refund policy summary" },
        // Accessibility
        ada_compliant: { type: "boolean", description: "True if the venue is ADA compliant" },
        elevator_access: { type: "boolean", description: "True if there is elevator access to upper floors" },
        ramp_locations: { type: "string", description: "Locations of accessibility ramps" },
        accessible_restrooms: { type: "boolean", description: "True if accessible restrooms are available" },
        wheelchair_seating_areas: { type: "string", description: "Dedicated wheelchair seating areas" },
        // Wedding & Party Specifics
        bridal_suite: { type: "boolean", description: "True if a private bridal suite (with mirrors and seating) is available" },
        groom_ready_room: { type: "boolean", description: "True if a groom's room or ready room is available" },
        indoor_ceremony_space: { type: "boolean", description: "True if an indoor vow exchange / ceremony space is available" },
        outdoor_ceremony_space: { type: "boolean", description: "True if an outdoor ceremony space is available" },
        grand_staircase: { type: "boolean", description: "True if there is a grand entrance staircase" },
        scenic_views: { type: "string", description: "Scenic views (cityscape, garden, waterfront, etc.)" },
        photography_locations: { type: "string", description: "Notable photography locations and landmarks on site" },
        // Audio / Visual / Tech
        builtin_sound_system: { type: "boolean", description: "True if a built-in professional sound system is provided" },
        wireless_microphones_count: { type: "number", description: "Number of wireless microphones (handheld + lapel)" },
        projection_or_led_walls: { type: "string", description: "Projection screens or LED video wall details" },
        custom_lighting: { type: "string", description: "Custom lighting options (uplighting, spotlights, gobos)" },
        guest_wifi: { type: "boolean", description: "True if high-speed guest Wi-Fi is provided" },
        livestream_hybrid_support: { type: "boolean", description: "True if livestreaming / hybrid event support is available" },
        band_dj_power_drops: { type: "boolean", description: "True if dedicated power drops for bands/DJs are provided" },
        // Dining & Hospitality
        onsite_commercial_kitchen: { type: "boolean", description: "True if a full on-site commercial kitchen is available" },
        event_coordination_services: { type: "string", description: "Maitre d' and event coordination services offered" },
        menu_tasting_room: { type: "boolean", description: "True if an on-site menu tasting room is available" },
        premium_bar_packages: { type: "string", description: "Premium bar package options" },
        specialty_cocktail_options: { type: "string", description: "Specialty / signature cocktail menu options" },
        late_night_food_setup: { type: "string", description: "Late-night snack or food truck setup options" },
        dessert_station_available: { type: "boolean", description: "True if dessert stations or Viennese tables are available" },
        // Vibe & Atmosphere
        dance_floor: { type: "string", description: "Dance floor details (permanent or portable, size)" },
        stage_dimensions: { type: "string", description: "Stage dimensions for bands or speakers" },
        coat_check_staffed: { type: "boolean", description: "True if a staffed coat check room is provided" },
        outdoor_fire_pits_heaters: { type: "boolean", description: "True if outdoor fire pits and heaters are available" },
        cigar_or_cocktail_lounge: { type: "boolean", description: "True if a dedicated cigar or cocktail lounge is available" },
        rooftop_or_terrace_access: { type: "boolean", description: "True if rooftop or garden terrace access is available" },
      },
