...
--
-- The iframe is intentionally sandboxed without `allow-same-origin`
-- (Chrome warns about sandbox escape and re-introducing it caused an
-- auth-token storage-lock re-render loop). That means it cannot read
-- parent auth/session storage, and `window.top` / `document.referrer`
-- / origin checks are unreliable in that context. Instead, the
-- dashboard (which IS authenticated) calls `issue_studio_preview_token`
-- to insert a row and gets back the row id; that id is appended to the
-- iframe URL as `?previewToken=`. The public route's loader calls
-- `verify_studio_preview_token` to confirm the row exists, isn't
-- expired, and is bound to the requested slug + its current owner.
--
-- The token id is a UUID v4 (~122 bits of entropy) — unguessable in
-- practice. The row stores no sensitive data; nothing is exposed to
-- anon clients except via the verify RPC, which returns only a boolean.
-- This deliberately replaces the HMAC-signed approach so the feature
-- works in deployments without `PRESENTATION_TOKEN_SECRET` set.
...
-- No direct table access — all reads/writes go through the SECURITY
-- DEFINER RPCs below. (Service role still bypasses RLS for any future
-- maintenance.)
...
-- Issue a token bound to the given slug, valid for 1 hour. The caller
-- must be authenticated as the slug's owner OR an admin; otherwise
-- NULL is returned. Returns the freshly-issued token id (a uuid).
...
BEGIN
  ...
  -- Opportunistic cleanup — keep the table small without a cron job.
...
-- Verify a token presented by the iframe. Returns TRUE only when:
--   * the token row exists and hasn't expired
--   * the token's slug matches the slug the iframe is loading
--   * the token's provider_id still matches the current owner of that
--     slug (defends against slug reassignment after issuance)
