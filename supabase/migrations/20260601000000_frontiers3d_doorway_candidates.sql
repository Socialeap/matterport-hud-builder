...
-- ------------------------------------------------------------
-- Bridges normalized Map-Oracle `properties` (PR-B1) into an
-- OPERATOR-CONTROLLED doorway/HUD discovery surface, so operators can
-- review/queue/surface Map-Oracle candidates WITHOUT any lead/client
-- binding, beacon creation, consent change, or money flow.
--
-- What this lands (all NET-NEW, strictly additive):
--   * _compose_hero_summary(property_id)        — hero line composer
--     (lifted verbatim from the staged doorway bridge; a future full
--      doorway bridge CREATE OR REPLACEs the same body — no conflict)
--   * compose_doorway_payload(property_id) -> jsonb — builds the HUD
--     discovery card from properties + child tables and RETURNS it
--     (does NOT write to agent_beacons — that beacon-write path is the
--      deferred full doorway bridge, intentionally NOT included here)
--   * doorway_candidates table — operator triage queue over properties
--     (status: new|queued|surfaced|dismissed; fully reversible)
--   * detect_doorway_candidates(limit) — operator-invoked, idempotent
--     stager (NO trigger, NO cron — nothing auto-fires)
--   * set_doorway_candidate_status(property_id, status) — operator
--     lifecycle control (admin-only)
--   * operator_doorway_candidates view (security_invoker=true) — the
--     admin-only operator surface
--
-- EXPLICITLY NOT in this migration (held / separate, approved PRs):
--   * agent_beacons changes (property_id/source/doorway_payload columns,
--     beacon write triggers, refresh-to-beacon worker) — the full
--     beacon doorway bridge.
--   * promote_property_to_beacon + the agent_beacons_consent_required
--     relaxation (B3 / Phase 2.5, DESTRUCTIVE — separate approval).
--   * B4 lead/client binding, client_providers, attribution, billing,
--     Stripe, platform fee — none touched.
--
-- Prerequisite: PR-B1 (properties + child tables + _safe_integer) — LIVE.
-- Safety: no DROP, no DELETE, no TRUNCATE, no destructive ALTER, no
-- change to any existing table/policy/RLS. Reversible (drop the new
-- objects). No cron scheduled by this migration.
-- ============================================================


-- ------------------------------------------------------------
-- 1. _compose_hero_summary(property_id)  (verbatim from staged bridge)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2. compose_doorway_payload(property_id) -> jsonb
--    The HUD discovery card built from properties + 6 child tables.
--    RETURNS the jsonb (read-only); writes nothing. This is the
--    property-centric counterpart of the staged beacon-writing
--    refresh_doorway_payload_for_property — same shape, no side effects,
--    no agent_beacons.
-- ------------------------------------------------------------
...
BEGIN
  ...
  RETURN v_payload;  -- NULL if the property does not exist
END;
$$;
...
-- ------------------------------------------------------------
-- 3. doorway_candidates — operator triage queue over properties
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.doorway_candidates (
  property_id      UUID PRIMARY KEY REFERENCES public.properties(id) ON DELETE CASCADE,
  status           TEXT NOT NULL DEFAULT 'new'
                     CHECK (status IN ('new','queued','surfaced','dismissed')),
  doorway_payload  JSONB,                 -- snapshot of the composed card
  notes            TEXT,
  created_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
  reviewed_by      UUID                    -- admin who last changed status
);
...
-- ------------------------------------------------------------
-- 4. detect_doorway_candidates(limit) — operator-invoked stager
--    Idempotent: inserts 'new' candidates for Map-Oracle properties not
--    yet queued, snapshotting the composed card. Re-running refreshes
--    the snapshot ONLY for still-'new' rows (operator decisions on
--    queued/surfaced/dismissed are preserved). NO trigger, NO cron.
-- ------------------------------------------------------------
...
REVOKE EXECUTE ON FUNCTION public.detect_doorway_candidates(INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.detect_doorway_candidates(INT) TO service_role, authenticated;


-- ------------------------------------------------------------
-- 5. set_doorway_candidate_status(property_id, status) — operator control
-- ------------------------------------------------------------
...
REVOKE EXECUTE ON FUNCTION public.set_doorway_candidate_status(UUID, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.set_doorway_candidate_status(UUID, TEXT) TO service_role, authenticated;


-- ------------------------------------------------------------
-- 6. operator_doorway_candidates — admin-only operator surface
--    security_invoker=true so the "Admins can read doorway_candidates"
--    RLS is ENFORCED (a normal view would BYPASS it). Non-admins get
--    zero rows. (Same pattern as B1 operator_failed_snapshots and A2
...
COMMENT ON VIEW public.operator_doorway_candidates IS
  'Admin-only (security_invoker) operator triage surface for Map-Oracle doorway candidates. Use detect_doorway_candidates() to stage and set_doorway_candidate_status() to queue/surface/dismiss.';

GRANT SELECT ON public.operator_doorway_candidates TO service_role, authenticated;

-- ============================================================
-- End of Track B / B2 — Doorway Candidates (property-centric).
-- ============================================================
