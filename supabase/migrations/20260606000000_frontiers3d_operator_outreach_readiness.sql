-- ============================================================
-- Frontiers3D — Map-Oracle Operator: outreach readiness aggregator
-- ------------------------------------------------------------
-- Read-only aggregator powering the admin operator UI. For each Map-Oracle
-- doorway candidate it returns business/location/website/email + email
-- enrichment confidence + promotion (beacon) status + latest outreach status.
--
-- SECURITY DEFINER with an internal admin gate so it can join across tables
-- with differing RLS (properties/contacts/enrichment + agent_beacons +
-- map_oracle_outreach_log) in one call; rows are visible to admins only.
--
-- Strictly additive. No writes, no destructive op. No B4/binding, Stripe,
-- billing, or Track A. No cron, no batch.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN ol.status IS NOT NULL        THEN ol.status     -- pending_render/queued/sent/suppressed/failed
      ELSE 'ready'                                          -- promoted + has email, no outreach yet
    END
...
-- ============================================================
-- End — operator outreach readiness aggregator (read-only, admin-only).
-- ============================================================
