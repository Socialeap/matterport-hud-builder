...
- **Files changed:**
  ...
  - `src/lib/atlas-curation.functions.ts` — NEW `inspectShowcaseRuntime` (read-only deployed vs current) + `republishCuratedShowcase` (Atlas-only; delegates to the shared helpers; **verifies the live runtime is older via `evaluateRuntimeUpgradeGate` before opening any PR**; opens upgrade PR on the SAME slug; sets `pr_open` so the existing #178 Approve & Publish merges/redeploys/re-attaches; never clears `deployed_url`/`presentation_url`; never changes activation).
  ...
  - `tests/atlas-runtime-upgrade.test.mjs` — NEW (20: U1–U5 version compare; U6–U10 guard; U11–U14 slug/data/URL/listing preservation; U15–U16 single-file patcher still rejects atlas + new message; U17–U18 no client-side secrets / dynamic-import boundary; U19–U20 downgrade gate + verify-before-PR wiring).
...
- **Review fixes:**
  - **P2 (GitHub-ChatGPT) — downgrade guard.** `republishCuratedShowcase` previously proceeded after only the structural `canRepublishShowcase` check, so a direct server call (or a click after a failed/`unknown` inspection) could regenerate a newer live folder with this build's older runtime. Added an authoritative server-side gate: it re-reads the deployed manifest's `runtime_version` and proceeds **only** when `evaluateRuntimeUpgradeGate` returns `upgrade_available` (strictly older). `current` / `ahead_of_build` / `unknown` (incl. no live URL) are rejected **before** any PR is opened, and these throw **without** marking the healthy published job `failed`. UI also tightened: the button is enabled only when an upgrade is confirmed available. Tests U19–U20.
- **Verification (rebased on main):** `tsc` **0**; `eslint` **0** (changed files); `vite build` OK (routeTree SSR-register drift reverted); `verify:html` PASS; `verify:no-secrets` PASS; `git diff --check` clean; `test:intelligence` **619/619** (20 new).
...
- **End-State Alignment:**
  ...
  - Boundaries preserved: single-file path still rejects `family=atlas` (canUpgrade false); **no downgrade — server-enforced runtime gate**; no auto-activation; secrets stay server-side (dynamic-import boundary test); slug/path/URL preserved; activation untouched; no Matterport SDK; no migration.
